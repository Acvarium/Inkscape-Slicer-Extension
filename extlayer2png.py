...
class ExportSlices(inkex.EffectExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument("--selected", type=inkex.Boolean, help="Export only selected")
    ...
    def effect(self):
        ...
        for node in nodes:
            ...
            if self.options.selected:
                usable_node = node.attrib["id"] in selected
            if usable_node:
                ...
                self.color_map[node.attrib["id"]] = ExportSlices.GREY
    ...
    def get_selected_nodes(self):
        selected = []
        selected_nodes = self.svg.selected
        for s in selected_nodes:
            # inkex.utils.debug(s.attrib["id"])
            selected.append(s.attrib["id"])
        return selected
